...
var RssSubmitModal = /*#__PURE__*/function (_Modal) {
  ...
  _proto.loadExistingFeed = function loadExistingFeed() {
    var _this = this;
    ...
    flarum_forum_app__WEBPACK_IMPORTED_MODULE_3___default().request({
      method: 'GET',
      url: flarum_forum_app__WEBPACK_IMPORTED_MODULE_3___default().forum.attribute('apiUrl') + '/user-rss-feeds' // 请求新的 API 端点
    }).then(function (response) {
      if (response.data.length > 0) {
        var userFeed = response.data[0]; // 使用当前用户的第一个 RSS Feed
        _this.feedId = userFeed.id;
        _this.url = userFeed.attributes.url;
        _this.siteTitle = userFeed.attributes.title;
        console.log('[RssSubmitModal] Found user feed:', userFeed);
      } else {
        console.log('[RssSubmitModal] No existing feed found for the user.');
      }
      _this.loading = false; // 加载完成
  ...
  _proto.content = function content() {
    ...
    return m("div", {
      ...
      oninput: function oninput(e) {
        _this2.siteTitle = e.target.value; // 捕获站点名称
        console.log('[RssSubmitModal] Updated siteTitle:', _this2.siteTitle);
      }
    })), m("div", {
      className: "Form-group"
      ...
      ,
      oninput: function oninput(e) {
        _this2.url = e.target.value; // 捕获 RSS 链接
        console.log('[RssSubmitModal] Updated URL:', _this2.url);
      }
    })), m("div", {
      className: "Form-group"
  ...
  _proto.onsubmit = function onsubmit(e) {
    ...
    var method = this.feedId ? 'PATCH' : 'POST'; // 如果有 feedId，则更新，否则创建
    var apiUrl = this.feedId ? flarum_forum_app__WEBPACK_IMPORTED_MODULE_3___default().forum.attribute('apiUrl') + ("/rss-feeds/" + this.feedId) : flarum_forum_app__WEBPACK_IMPORTED_MODULE_3___default().forum.attribute('apiUrl') + '/rss-feeds';
